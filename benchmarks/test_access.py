...
_TREE: Final[sexp.SExp] = sexp.parse(MEDIUM)
_LAST_IDX: Final[int] = len(_TREE) - 1
...
def test_key_hit(benchmark: BenchmarkFixture) -> None:
    benchmark.pedantic(lambda: _TREE["pos"], iterations=1000, rounds=100)


def test_key_miss(benchmark: BenchmarkFixture) -> None:
    def _miss() -> None:
...
def test_iter_children(benchmark: BenchmarkFixture) -> None:
