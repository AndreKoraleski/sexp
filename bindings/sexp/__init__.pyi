...
Contains inline class and function definitions so that IDEs (Pylance, pyright,
mypy) can resolve method signatures and docstrings for ``sexp.SExp``,
``sexp.SExpNode`` and ``sexp.parse`` without needing to locate the ``_sexp``
C extension stub separately.
"""
